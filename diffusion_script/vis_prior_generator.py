...
class VisPriorGenerator():
    ...
    def update_prior_bank_with_images(self, annotation, im_folder):
        ...
        for img_obj in tqdm(coco.loadImgs(imgIds)):
    ...
    def sample_image_visual_priors(self, num_layouts, pixel_size, morphology_kernal_size=0):
        ...
        if num_layouts>len(self.prior_bank["image_visual_priors"]):
            prior_indices = list(range(num_layouts))
        else:
            raise ValueError("num_layouts should not exceed number of images in the dataset")
        image_visual_priors = []
